...
2026-06-15 — [FIX] Soft Opening Jun 11→Jun 8 sweep done across 11 files (brand brain ×2, offer-engine, campaign-performance, video-ads, lead-gen, URBN-campaign-plan, brief-2026-06-14, Program-Architecture ×2, creative-handoff). Left week-2-plan.md + raw June-Marketing-Plan.md untouched — their Jun 11 is baked into Thu/countdown structure (Jun 8 is a Monday); flagged for re-dating. Lesson logged + routed to LINT.md (real-world event facts are Jap's to confirm; canonical files can hold stale planned dates).
2026-06-15 — [FIX] URBN Soft Opening date confirmed by Jap = Jun 8 (not Jun 11). My earlier inference from the canonical brand brain was wrong — it held a stale planned date. brief.md + STATE reverted to Jun 8. Stale "Jun 11" now remains in Main-Brand-Brain.md, offer-engine.md, and recent campaign plans (week-2/video-ads/lead-gen/URBN-campaign-plan/brief-2026-06-14) — to correct.
2026-06-15 — [OPS] P1-1 (half): legacy OneDrive vault OUTPUTS archived → OUTPUTS-ARCHIVED (157 files, reversible rename) so stray writes from old chats break loudly instead of landing silently. Marker note `_VAULT-MOVED-READ-ME.md` left at legacy root pointing to C:\Jnoah\vault. All legacy OUTPUTS content already mirrored in canonical (dashboard drift = 0). Remaining: Jap retires the old Cowork chat still attached to the OneDrive project. Surfaced an unresolved fact conflict — URBN Soft Opening date: Jun 8 (brief.md/STATE) vs Jun 11 (weekly brief-2026-06-14.md).
2026-06-15 — [SYSTEM] — lint: dashboard drift warning caught 10 legacy strays — URBN rebrand campaign files (THIS-WEEK-*, URBN-campaign-plan, brief-2026-06-14, autoreply-programs-faq, event/group-chat/ig-captions/member-update) a parallel chat wrote to the OneDrive legacy vault. Copied to canonical vault (Node fs, size-verified), committed (3e76d59). Drift cleared. Root cause = P1-1 (old chat still live-writing to OneDrive) — needs that chat retired.
2026-06-15 — [OPS] P2-5 closed: Jnoah OS start/restart scripts added (start-os.ps1/.bat, stop-os.ps1) — frees port 3000, launches dev detached + logged, waits for ready. README rewritten (run/restart, login startup, control-layer overview, localhost-only security note). Dashboard no longer dies silently with no way back. App repo committed locally (no git remote yet — Jap's call).
...
2026-06-12 — [CLIENT UPDATE] Jemina Apostol — training environment confirmed (CrossFit/URBN gym, no selectorized machines), profile equipment list updated. program-state.md created with new "Program Type: Prehab (situational, client-specific)" field — Block 1 (Sessions 1-4, Foundational Strength & Stability) marked, ships per-session not per-week. Registry flag updated. Exercise menu for S1-4 revised for no-machine equipment (next message).
2026-06-11 — [CLIENT] New client Jemina Apostol added — pre-ACL repair prehab. Profile created (/MEMORY/clients/Jemina-Apostol/Jemina-Apostol.md), references/check-ins placeholders added, registry updated. 10-session program planned with feedback loop every 3 sessions, to be built off /RESEARCH/pre-acl-ops/deep-research-report-PRE-ACL-OPS.md. Brand, service tier, and procedure details still pending confirmation.
...
2026-06-12 — [BUILD] Marketing engine unification COMPLETE (Option B) — /ENGINES/marketing/ now a 5-file suite: marketing-engine.md (master, 9-stage lifecycle, 3 gates), strategy.md (brand roles/STP/JTBD/CEP/Binet&Field — endorsed-architecture rec from research OVERRULED by vault law), experiments.md (hypothesis cards + decision rules), performance.md (AARRR per brand, locked activation events, OMTM, Gate 3 learning loop), campaign.md repointed as execution arm. State: /MEMORY/marketing/strategy-state.md (cards DRAFT) + experiments-log.md. First 2 agents: marketing-strategist, campaign-performance-analyst. ROUTER (6 rows), CLAUDE.md, AGENTS.md updated. Research registered in INDEX (slug: marketing). Premortem: /MEMORY/pre-mortem-marketing-engine.md — GO with 3 follow-ups (coach-jap brand file, DRAFT card validation, watch strategy-state staleness).
...
2026-06-08 — [REGISTRY] Registry updated: Em Tan Phase 2/D5–D8 corrected, Arianna/Fame/Katrina Mae flags updated (body metrics on file, intake pending), Katrina Mae legal name resolved, last-updated date synced.
2026-06-08 — [CLIENT UPDATE] Arianna Ramos, Fame Florendo, Jen Linzaga, Katrina Mae Carandang — profiles updated from Goal Review Forms. Body metrics, personal details, contact info populated. Flags: Arianna name spelling, all 4 on Urban Strong forms (brand confirm needed), Jen age/height corrected, Katrina Mae legal name confirmed as Katrina Mae S. Carandang.
...
2026-06-12 — [PROGRAM] Jemina Apostol — Session 1 and Session 2 of Pre-ACL Prehab Block 1 (Foundational Strength & Stability) drafted: no-machine equipment, bilateral squat/bridge/hip-stability/core, RPE 4-6 + pain VAS <3-4/10 gate, single-leg balance baseline test added to warm-up, S2 carries built-in progression rule. Format B (coach-jap-programming-format SKILL.md, SESSION DEBRIEF close) used and flagged vs. training-program.md's Format B variant; standalone Mobility block also flagged. Awaiting Jap's run + adjustments before S3/S4 build.
2026-06-12 — [FLAG] Program file naming/structure inconsistency flagged for future fix across all client programs — some clients get standalone "C[cycle]-Day[N]-[descriptor].md" files (Yury C13-Day5, Aira C3-Day5), others get one "[Client]-Cycle[N]-Week[N].md" file with embedded Session 1/2/3 (Jozen). Standardization pass not yet scheduled — see /MEMORY/lessons-learned.md PENDING entry. Proceeding with Jozen Cycle 4 Week 1 Session 1 for today as-is.

## 2026-06-12 — Full marketing ops rerun across both brands (approved by Jap)
- Strategy validation: segment cards updated from real data. 6FIX buyer = "Referred Busy Professional" (med tech cohort, VALIDATED); URBN = "Hyrox-Influenced Gym Returnee"; "Event Deadline" segment retired until funnel exists. Coach Jap removed as third brand — two brands only.
- URBN: Week 3–4 improved plan + Grand Opening arc (Jul 1–11) built (week-3-4-plan.md, grand-opening-plan.md). GO confirmed Jul 11. Founding Rate final-call push added Jun 19–22.
- Restart PH: Social Growth Pipeline CLOSED EARLY — 0 discovery calls; funnel assumption failed. Superseded by DM-First Pipeline (Jun 16–Jul 13): DM "RESTART" only public CTA, call inside DM flow, OMTM = DM volume, EXP-2026-06-1 attached (first experiment card).
- 6FIX: June launch campaign ARCHIVED (died in DRAFT — lesson logged + routed to campaign.md activation rule). New campaign: 2026-07 6FIX Public Launch, Cohort 2 starts Jul 27 (Option A locked by Jap). Referral-engine-first architecture. OPEN: price decision before Jul 1 (₱6,000 recommended).

## 2026-06-12 — 6FIX Cohort 2 price locked: 6,000 PHP
- Regular price 6,000 PHP (3,000 was Cohort-1-only launch price). Buddy mechanic kept: 10% off both at enrollment together (5,400 each). Public copy never frames 6,000 as an increase — it is the price.

## 2026-06-12 — URBN offer naming locked + agent strategy reviews complete
- Pioneer rate = new members (soft-opening marketing onward, limited 20, runs toward Grand Opening Jul 11). Founding Member Rate = previous Urban Strong clients only — closes Jun 22, NO extension. Conflation fixed in brand file, week-3-4-plan, grand-opening-plan. Lesson logged + routed.
- marketing-strategist agent ran both brand reviews: Restart CEPs claimed by owner campaigns, OMTM confirmed (DM volume, both DM keywords count); URBN rebrand campaign claims "functional training" CEP, OMTM set (trial bookings/week, pivot to conversion after Jul 18 if trials flow but memberships do not). strategy-state.md now PARTIALLY VALIDATED.
- Post-Grand-Opening URBN referral campaign GREENLIT (late July, claims "My friend trains there" CEP) — build after GO data lands.

## 2026-06-14 — Program file naming convention unified (lint fix)
- Fixed self-contradiction in training-program.md: "Output destination" section (lines 760-765, stale `YYYY-MM-DD-*.md` naming) rewritten to match the canonical "Output file format rule" (2026-05-30) — `[ClientName]-[CycleLabel]-Week[N].md` for weekly blocks (default), `[ClientName]-[CycleLabel]-Week[N]-Day[N]-[descriptor].md` for rare 1-2x/week clients.
- ROUTER.md Programs row was a third, conflicting naming scheme (`YYYY-MM-DD-program-name.md`) — aligned to the same convention.
- Resolves 2026-06-12 lessons-learned PENDING item (Day-5 / per-client naming inconsistency). Existing files not retroactively migrated.
2026-06-14 — [PROGRAM] 6FIX Phase 2 Week 1 (Reset & Rebuild) built for Arianna, Fame, Katrina Mae — individualized, home/minimal-equip, 2x/wk + running conditioning, individual nutrition targets, Trainerize calendar/message/program blocks included.
2026-06-14 — [PROGRAM] 6FIX Phase 2 COMPLETE plans (full 6-wk training + meal plan) built for Jen Linzaga, Katrina Mae Carandang, Arianna Ramos from Phase 2 questionnaire intakes. Meal plans at 60/30/10 per Jap directive (fat-floor caveat noted per file). Home/minimal+running. Flags: Jen 2018 surgery type + sleep 5h; Katrina Mae stress 8/10 + real goal weight; Arianna PCOS + training-time barrier.
2026-06-14 — [ADMIN] Renamed client Karen → Katrina Mae Carandang system-wide: client + program folders, profile, program-state, registry, and both Phase 2 plan files. Legal name retained as Katrina Mae S. Carandang. Historical snapshots (checkpoint, pre-mortem) left as audit trail.
2026-06-14 — [ADMIN] Cleanup: deleted superseded Phase 2 Week-1 files for Arianna + Katrina Mae (complete plans supersede). Fame's Week-1 kept (no complete plan yet). Scrubbed residual 'Karen' refs from historical snapshots (checkpoint + pre-mortem) → Katrina Mae.
2026-06-14 — [NUTRITION] Client-facing 6FIX Phase 2 nutrition+training PDFs issued for Jen, Arianna, Katrina Mae (branded Restart, /OUTPUTS/nutrition/[client]/). Whole-food protein-forward targets (~200-220g P, fat at healthy floor) after 60/30/10 found unreachable whole-food-only. Rules: protein-first, no liquid sugar, fiber carbs only, MyFitnessPal logging. 12km/wk + gym recommendation. 3 PH meal variants each. Brand fonts approximated in render (Caladea/Carlito); colors exact.
